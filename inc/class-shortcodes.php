...
class Shortcodes {
	...
	public function handle_cart_items_loop( $atts, string $content = '' ): string {
		...
		if ( ! $cart || empty( $cart->cart_data ) ) {
			return Data_Fetcher::resolve( '{ff_cart_items_table}' );
		}
		...
		Data_Fetcher::enqueue_fluentcart_assets();
		Data_Fetcher::enqueue_cart_live_assets();

		$output = '';
		foreach ( $data as $item ) {
			Data_Fetcher::set_cart_item_context( $item );
			$processed = do_shortcode( $content );
			$processed = Data_Fetcher::resolve_content_tokens( $processed );
			$output   .= $processed;
		}
		Data_Fetcher::reset_cart_item_context();
	...
	public function handle_customer_orders_loop( $atts, string $content = '' ): string {
		...
		foreach ( $orders as $order ) {
			Data_Fetcher::set_order_context( $order );
			$processed = do_shortcode( $content );
			$processed = Data_Fetcher::resolve_content_tokens( $processed );
			$output   .= $processed;
		}
		Data_Fetcher::reset_order_context();
		return $output;
	}
	...
	public function handle_product( $atts, string $content = '', string $tag = '' ): string {
		...
		return Data_Fetcher::resolve( $token, $id );
	}
	...
	public function handle_cart( $atts, string $content = '', string $tag = '' ): string {
		$token = $this->braced_token_from_shortcode( $tag );
		return Data_Fetcher::resolve( $token );
	}
	...
	public function handle_button( $atts, string $content = '', string $tag = '' ): string {
		...
		$html = Data_Fetcher::resolve( $token, $id );

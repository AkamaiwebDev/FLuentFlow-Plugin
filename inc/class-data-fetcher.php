...
class Data_Fetcher {
	...
	private static function resolve_product_id( ?int $context_id = null ): ?int {
		$post_type = self::get_product_post_type();

		if ( null !== $context_id && $context_id > 0 ) {
			if ( get_post_type( $context_id ) === $post_type ) {
				return $context_id;
			}
		}
	...
	private static function get_customer_model( ?int $context_id = null ): ?object {
		...
		if ( null !== $context_id && $context_id > 0 ) {
			$customer = \FluentCart\App\Models\Customer::find( $context_id );
	...
	private static function resolve_cart_item_count(): string {
		...
		$cart = self::get_cart_model();
		if ( ! $cart || empty( $cart->cart_data ) ) {
			return '<span data-ffbb-token="cart_item_count">0</span>';
		}
	...
	private static function resolve_cart_total(): string {
		...
		return sprintf(
			...
			self::format_price( (float) $total / 100 )
		);
	}
	...
	private static function resolve_cart_items_table(): string {
		...
		foreach ( $data as $item_id => $item ) {
			...
			$image      = esc_url( $item['featured_media'] ?? '' );
			$qty        = (int) ( $item['quantity'] ?? 1 );
	...
	private static function resolve_cart_item_image(): string {
		...
		if ( ! $image ) {
			$image = self::resolve_cart_item_image_from_variation( $item );
		}
		if ( ! $image ) {
		...
		if ( ! $image ) {
			$image = self::get_fluentcart_placeholder();
		}
		if ( ! $image ) {
			return self::fallback();
		}
	...
	private static function resolve_cart_item_image_from_variation( array $item ): string {
		...
		try {
			...
			if ( $variation ) {
				$thumb = $variation->thumbnail;
				if ( $thumb ) {
					return $thumb;
				}
				if ( $variation->product && $variation->product->thumbnail ) {
					return $variation->product->thumbnail;
				}
			}
		} catch ( \Exception $e ) {
			do_action( 'ffbb_debug', 'resolve_cart_item_image_from_variation error: ' . $e->getMessage() );
		}
		return '';
	}
	...
	private static function get_subscription_model( ?int $context_id = null ): ?object {
		...
		if ( $context_id && $context_id > 0 ) {
			$sub = \FluentCart\App\Models\Subscription::find( $context_id );
			return $sub ?: null;
		}
		$user_id = get_current_user_id();
		if ( ! $user_id ) {
		...
		$sub = \FluentCart\App\Models\Subscription::where( 'customer_id', $customer->id )
	...
	private static function get_coupon_model( ?int $context_id = null ): ?object {
		...
		if ( $context_id && $context_id > 0 ) {
			$coupon = \FluentCart\App\Models\Coupon::find( $context_id );
			return $coupon ?: null;
		}
		return null;
	}
	...
	private static function get_variation_id_for_product( ?int $product_id = null ): int {
		$product = self::get_product_model( $product_id );
		if ( ! $product ) {
			return 0;
		}
	...
	public static function handle_ajax_cart_data(): void {
		...
		$total = method_exists( $cart, 'getEstimatedTotal' )
			? self::format_price( (float) $cart->getEstimatedTotal() / 100 )
			: self::format_price( $subtotal / 100 );
	...
	public static function enqueue_cart_live_assets(): void {
		...
		$asset_url = FFBB_URL . 'inc/assets/ffbb-cart-live.js';
